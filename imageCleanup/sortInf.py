...
root_folder_path = "small"
sorted_folder_path = "face_sorted"
non_sorted_folder_path = "non_face_sorted"

if not os.path.exists(sorted_folder_path):
    os.makedirs(sorted_folder_path)
    
if not os.path.exists(non_sorted_folder_path):
    os.makedirs(non_sorted_folder_path)
...
for root, dirs, files in os.walk(root_folder_path):
    for filename in files:
        if filename.endswith(".jpg"):
            ...
            if score >= 0.58:
                nonface += 1
                shutil.copy(img_path, os.path.join(non_sorted_folder_path, f"{filename}"))
            else:
                face += 1
